    ...
    <section className="mt-20 md:mt-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
        {reviews.map((review, i) => (
          <div key={i} className="bg-white p-6 md:p-8 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] relative flex flex-col justify-between hover:-tranneutral-y-1 transition-transform duration-300">
            <div className="absolute top-0 left-0 w-2 h-full bg-red-600"></div>
            <div className="mb-6 pl-2">
              <div className="flex text-amber-500 text-lg mb-4">
                <span>★</span><span>★</span><span>★</span><span>★</span><span>★</span>
              </div>
              <p className="text-neutral-700 font-medium leading-relaxed italic">&quot;{review.text}&quot;</p>
            </div>
            <div className="font-black text-black uppercase tracking-wide pt-4 border-t-2 border-neutral-100 pl-2">
              {review.name}
            </div>
          </div>
        ))}
